...
public class BlogServiceImpl implements BlogService {
    ...
    public List<Blog> getListRecommendBlogTop(Integer size) {
        return blogDao.getListRecommendBlogTop(size);
    }
    ...
    public int updateBlog(Blog blog) {
        blog.setUpdateTime(new Date());
        //将标签的数据存到t_blogs_tag表中，此处可能会重复，可改为，删除blog_tag表中的该blogid的，然后再批量插入。（）
        List<Tag> tags = blog.getTags();
        BlogAndTag blogAndTag = null;
        for (Tag tag : tags) {
    ...
    @Override   //编辑博客
    public int updateBlogView(Blog blog) {

        return blogDao.updateBlogView(blog);
    }
